...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if checkpoint:
        ...
        if 'P' in checkpoint_data:
            gaussians.P = checkpoint_data['P'].to('cuda')
        gaussians.restore(model_params, opt)
        if gaussians.triplane is not None:
            triplane_dir = os.path.join(scene.model_path, "point_cloud", f"iteration_{first_iter}")
            gaussians.load_triplane_weights(triplane_dir)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        camera_pose = gaussians.get_RT(camera=viewpoint_cam)
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, camera_pose=camera_pose)
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations):
                ...
                torch.save(checkpoint_data, scene.model_path + "/chkpnt" + str(iteration) + ".pth")
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in tqdm(enumerate(DataLoader(config['cameras'], shuffle=False, batch_size=None, num_workers=8)), total=len(config['cameras'])):
                    ...
                    eval_camera_pose = scene.gaussians.get_RT(camera=viewpoint)
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs, camera_pose=eval_camera_pose)["render"], 0.0, 1.0)
