...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.P = None
        self.cam_uid_map = None
        self.cameraid_to_pose_idx = None
        self.cam_scheduler_args = None
        self.optim_pose_enabled = False
        self.triplane = None
        self.low_triplane = None
        self.xyz_cano = None
    ...
    def init_RT_seq(self, cam_list, trainable: bool = True):
        """Initialize camera poses from camera list. Set trainable=False to freeze poses."""
        from utils.pose_utils import get_tensor_from_camera

        cameraid_to_pose_idx = {}
        poses = []
        self.cam_uid_map = {}
        self.cameraid_to_pose_idx = {}

        for cam in cam_list:
            spatial_id = cam.camera_id if cam.camera_id is not None else cam.colmap_id

            if spatial_id not in cameraid_to_pose_idx:
                p = get_tensor_from_camera(cam.world_view_transform.transpose(0, 1))
                pose_idx = len(poses)
                poses.append(p)
                cameraid_to_pose_idx[spatial_id] = pose_idx
            else:
                pose_idx = cameraid_to_pose_idx[spatial_id]

            self.cam_uid_map[cam.uid] = pose_idx

        self.cameraid_to_pose_idx = cameraid_to_pose_idx
        poses = torch.stack(poses)
        self.optim_pose_enabled = trainable
        self.P = poses.cuda().requires_grad_(trainable)
        print(f"Initialized {len(poses)} unique spatial camera poses from {len(cam_list)} cameras (grouped by camera_id)")

    def get_RT(self, camera=None):
        if self.P is None:
            return None

        if camera is not None:
            spatial_id = camera.camera_id if camera.camera_id is not None else camera.colmap_id
            if spatial_id in self.cameraid_to_pose_idx:
                idx = self.cameraid_to_pose_idx[spatial_id]
                return self.P[idx]
    ...
    def restore(self, model_args, training_args):
        ...
        self.optim_pose_enabled = training_args.optim_pose and (self.P is not None)
        if self.P is not None:
            self.P.requires_grad_(self.optim_pose_enabled)
    ...
    def get_features(self, cam_pos=None):
        if self.triplane is not None and cam_pos is not None:
            xyz = self.get_xyz
            ...
            view_dir_cano = torch.bmm(
                torch.bmm(self.face_orien_mat_cano[self.binding], self.face_orien_mat[self.binding].transpose(2, 1)),
                view_dir_def[..., None]
            ).squeeze(-1) * self.face_scaling_cano[self.binding] / self.face_scaling[self.binding]
            view_dir_cano = view_dir_cano / view_dir_cano.norm(dim=1, keepdim=True)
            xyz_cano = self.xyz_cano
            base_rgb = self.low_triplane(xyz_cano)
            rgb = self.triplane(xyz_cano, view_dir_cano)
            return 0.5 * base_rgb + rgb
    ...
    def training_setup(self, training_args):
        ...
        if self.P is not None and self.optim_pose_enabled:
            l.append({'params': [self.P], 'lr': training_args.camera_pose_lr, "name": "pose"})

        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)

        if self.P is not None and self.optim_pose_enabled:
            self.cam_scheduler_args = get_expon_lr_func(
                lr_init=training_args.camera_pose_lr,
                lr_final=training_args.camera_pose_lr_final,
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
            elif param_group["name"] == "pose" and self.cam_scheduler_args is not None:
                param_group['lr'] = self.cam_scheduler_args(iteration)
        return lr
    ...
    def load_ply(self, path, **kwargs):
        ...
        expected_f_rest = 3 * (self.max_sh_degree + 1) ** 2 - 3
        if len(extra_f_names) == 0:
            # Triplane checkpoints store appearance in triplane.pth, not SH rest coeffs.
            features_extra = np.zeros((xyz.shape[0], 0, 3), dtype=np.float32)
        elif len(extra_f_names) == expected_f_rest:
            ...
            raise ValueError(
                f"Unexpected f_rest count in {path}: got {len(extra_f_names)}, expected 0 (triplane) or {expected_f_rest}"
            )
        ...
        self.active_sh_degree = 0 if len(extra_f_names) == 0 else self.max_sh_degree
