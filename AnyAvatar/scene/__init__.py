    ...
    def __init__(self, args : ModelParams, gaussians : Union[GaussianModel, FlameGaussianModel], load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if gaussians.binding != None:
            self.gaussians.load_meshes(
                scene_info.train_meshes,
                scene_info.test_meshes,
                scene_info.tgt_train_meshes,
                scene_info.tgt_test_meshes,
        ...
        all_cameras = self.train_cameras[resolution_scales[0]].copy()
        if len(self.test_cameras[resolution_scales[0]]) > 0:
            all_cameras.extend(self.test_cameras[resolution_scales[0]])
        self.gaussians.init_RT_seq(all_cameras, trainable=args.optim_pose)
