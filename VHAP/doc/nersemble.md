...
--target-camera-id 222200037
```

Optional arguments

- `--target-camera-id 222200037`: set the world origin to camera `222200037` during export. This is required to adapt the exported dataset to the second-stage drivable avatar pipeline, which expects the coordinate system to be camera-centered rather than FLAME-centered.
