...
class TrackedFLAMEDatasetWriter:
    def __init__(self, cfg_model: ModelConfig, src_folder: Path, tgt_folder: Path, mode: Literal['mesh', 'param'], epoch: int = -1):
        ...
        # Relocate world origin and return the transformation matrix to modify camera poses.
        self.target_camera_id = cfg_model.target_camera_id
        self.M = self.relocate_flame_meshes(self.flame_params, target_camera_id=self.target_camera_id)

        print("Initializing FLAME model...")
        self.flame_model = FlameHead(cfg_model.n_shape, cfg_model.n_expr, add_teeth=True)
    
    def relocate_flame_meshes(self, flame_param, target_camera_id=None):
        """
        Relocate the world origin and return the transformation matrix to modify camera poses.

        If target_camera_id is set, the target camera is moved to the origin and FLAME
        translation is updated to preserve its relative offset to that camera.
        Otherwise, the FLAME center is moved to the origin (original behavior).

        Args:
            flame_param: FLAME parameters dict (modified in place)
            target_camera_id: Camera ID to place at the origin. If None, use FLAME center.
        ...
        reference_pos = T_mean
        use_camera_origin = False

        if target_camera_id is not None:
            target_camera_pos = None
            for frame in self.db['frames']:
                camera_id = frame.get('camera_id', None)
                if camera_id is not None and str(camera_id).lower() == str(target_camera_id).lower():
                    transform_matrix = np.array(frame['transform_matrix'])
                    if transform_matrix.shape == (4, 4):
                        target_camera_pos = transform_matrix[:3, 3]
                    elif transform_matrix.shape == (3, 4):
                        transform_4x4 = np.eye(4)
                        transform_4x4[:3, :] = transform_matrix
                        target_camera_pos = transform_4x4[:3, 3]
                    else:
                        print(f"Warning: Unexpected transform_matrix shape: {transform_matrix.shape}")
                        continue

                    use_camera_origin = True
                    print(f"Found target camera '{camera_id}' at position: {target_camera_pos}")
                    break

            if use_camera_origin and target_camera_pos is not None:
                reference_pos = torch.tensor(target_camera_pos, dtype=torch.float32)
            else:
                print(
                    f"Warning: Target camera '{target_camera_id}' not found in database. "
                    "Falling back to FLAME center."
                )

        M = torch.eye(4)
        M[:3, 3] = -reference_pos

        if use_camera_origin:
            flame_param['translation'] = (Ts - reference_pos).numpy()
            print("Relocated world origin to target camera:")
            print(f"  Target camera position: {reference_pos.numpy()}")
            print(f"  Original FLAME center (mean): {T_mean.numpy()}")
            print(f"  Relative position (FLAME - camera): {(T_mean - reference_pos).numpy()}")
        else:
            flame_param['translation'] = (Ts + M[:3, 3]).numpy()
            print("Relocated world origin to FLAME center:")
            print(f"  FLAME center (mean): {T_mean.numpy()}")
    ...
    def write(self):
        ...
        for i, frame in tqdm(enumerate(self.db['frames']), total=len(self.db['frames'])):
            ...
            # modify camera extrinsics according to the relocation transform
    ...
    def write_canonical_flame_param(self):
        if self.target_camera_id is not None:
            translation = np.mean(self.flame_params['translation'], axis=0, keepdims=True)
        else:
...
def main(
    ...
    """Export tracked FLAME results as a NeRF/3DGS dataset.

    Args:
        target_camera_id: Camera ID to place at the world origin during export.
            Overrides the value in config.yml when set.
    """
    ...
    if target_camera_id is not None:
        cfg.model.target_camera_id = target_camera_id
