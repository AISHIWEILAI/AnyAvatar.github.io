...
class FlameTracker:
    ...
    def triangulate_camera_rays(self, camera_centers, camera_directions):
        """
        Find the best intersection point of multiple camera rays via least squares.

        Minimizes:
        min_X sum_i || (I - D_i D_i^T)(X - P_i) ||^2

        Args:
            camera_centers: [N, 3] camera center positions
            camera_directions: [N, 3] unit viewing directions

        Returns:
            intersection_point: [3] triangulated point
        """
        ...
        for i in range(N):
            P_i = camera_centers[i]
            D_i = camera_directions[i]
            I_minus_DD = torch.eye(3, device=device, dtype=dtype) - torch.outer(D_i, D_i)
            A = A + I_minus_DD
            B = B + I_minus_DD @ P_i

        try:
            intersection_point = torch.linalg.solve(A, B.unsqueeze(1)).squeeze(1)
        except Exception:
            intersection_point = torch.linalg.pinv(A) @ B

        return intersection_point

    def solve_frustum_intersection_qp(self, camera_params_list, initial_point, z_near=0.1, z_far=10.0):
        """
        Project a point into the intersection of camera frustums using QP.

        minimize:   (1/2) * ||x - x_0||^2
        subject to: A @ x <= b

        Args:
            camera_params_list: List of dicts with keys:
                - 'K': [3, 3] intrinsic matrix
                - 'extrinsic': [4, 4] w2c transform
                - 'image_size': (H, W)
            initial_point: [3] initial point, usually from triangulation
            z_near: near plane distance
            z_far: far plane distance

        Returns:
            point: [3] constrained point in world coordinates
        """
        import cvxpy as cp

        x_0 = initial_point.detach().cpu().numpy()
        x = cp.Variable(3)
        objective = cp.Minimize(0.5 * cp.sum_squares(x - x_0))
        constraints = []

        for cam_param in camera_params_list:
            K = cam_param['K']
            extrinsic = cam_param['extrinsic']
            image_size = cam_param['image_size']

            R = extrinsic[:3, :3].detach().cpu().numpy()
            t = extrinsic[:3, 3].detach().cpu().numpy()
            ...
            z_cam_init = r2 @ x_0 + t2
            is_opengl = z_cam_init < 0

            if is_opengl:
                constraints.append(-r2 @ x <= z_far + t2)
                constraints.append(r2 @ x <= -z_near - t2)
                constraints.append((r0 + tan_half_fov_x * r2) @ x <= -t0 - tan_half_fov_x * t2)
                constraints.append((-r0 + tan_half_fov_x * r2) @ x <= t0 - tan_half_fov_x * t2)
                constraints.append((r1 + tan_half_fov_y * r2) @ x <= -t1 - tan_half_fov_y * t2)
                constraints.append((-r1 + tan_half_fov_y * r2) @ x <= t1 - tan_half_fov_y * t2)
            else:
                constraints.append(-r2 @ x <= -z_near + t2)
                constraints.append(r2 @ x <= z_far - t2)
        ...
        solution = torch.tensor(x.value, device=initial_point.device, dtype=initial_point.dtype)
        return solution
...
class GlobalTracker(FlameTracker):
    ...
    def init_params(self):
        ...
        # Initialize translation using triangulation from cameras
        if self.calibrated and len(self.dataset.camera_params) > 1:
            camera_centers = []
            camera_directions = []
            is_w2c = self.dataset.cfg.target_extrinsic_type == 'w2c'

            for camera_id in self.dataset.camera_ids:
                camera_param = self.dataset.camera_params[camera_id]
                extrinsic = torch.tensor(camera_param['extrinsic'], dtype=torch.float32)

                if extrinsic.shape[0] == 3:
                    extrinsic_4x4 = torch.eye(4, device=self.device, dtype=extrinsic.dtype)
                    extrinsic_4x4[:3, :] = extrinsic.to(self.device)
                ...
                camera_dir = camera_dir / (torch.norm(camera_dir) + 1e-8)
                camera_centers.append(camera_pos)
            ...
            if len(camera_centers) > 1:
                ...
                flame_center_unconstrained = self.triangulate_camera_rays(
                    camera_centers,
                    camera_directions,
                )
                ...
                flame_center_constrained = self.solve_frustum_intersection_qp(
                    camera_params_list,
                    flame_center_unconstrained,
                    z_near=0.1,
                    z_far=10.0,
                )

                projection_distance = torch.norm(
                    flame_center_constrained - flame_center_unconstrained
                ).item()

                self.translation.data.copy_(
                    flame_center_constrained.unsqueeze(0).expand(self.n_timesteps, -1).clone()
                )

                self.logger.info("Initialized FLAME translation with frustum constraint:")
                self.logger.info(f"  Number of cameras: {len(camera_centers)}")
                self.logger.info(
                    f"  Unconstrained (triangulation): {flame_center_unconstrained.cpu().numpy()}"
                )
                self.logger.info(
                    f"  Constrained (projected): {flame_center_constrained.cpu().numpy()}"
                )
                self.logger.info(f"  Projection distance: {projection_distance:.6f}")
